#ifndef MAX_HEAP_REAL_H
#define MAX_HEAP_REAL_H
...
inline VertexCostReal getFirstReal(std::vector<VertexCostReal>& H, robin_hood::unordered_map<usize, usize>& idx_H) {
...
inline void bubleUpReal(std::vector<VertexCostReal>& H, robin_hood::unordered_map<usize, usize>& idx_H, usize w, realT weight) {
...
inline void bubbleDownReal(std::vector<VertexCostReal>& H, robin_hood::unordered_map<usize, usize>& idx_H, usize w, realT weight) {
    // Update cost in max heap
    usize heap_idx = idx_H.at(w);
...
inline void insertHeapReal(std::vector<VertexCostReal>& H, robin_hood::unordered_map<usize, usize>& idx_H, usize vertex, realT cost) {
    const usize size = H.size();
    ...
    bubleUpReal(H, idx_H, vertex, cost);
}

// Remove an element from the heap H and idx_H and adjust the heap
inline void removeElementReal(std::vector<VertexCostReal>& H, robin_hood::unordered_map<usize, usize>& idx_H, usize w) {
    ...
    if (heap_idx > 0 && H[heap_idx].second > H[(heap_idx - 1) / 2].second) {
        bubleUpReal(H, idx_H, H[heap_idx].first, H[heap_idx].second);
    } else {
        bubbleDownReal(H, idx_H, H[heap_idx].first, H[heap_idx].second);
    }
}
